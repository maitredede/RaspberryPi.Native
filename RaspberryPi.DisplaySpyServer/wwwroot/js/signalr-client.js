...
class HttpConnection {
    constructor(url, options = {}) {
        ...
        this.logger = options.logger || new Loggers_1.NullLogger();
...
class HubConnection {
    constructor(connection, logger = new Loggers_1.NullLogger(), protocol = new JsonHubProtocol_1.JsonHubProtocol()) {
        this.connection = connection;
        this.logger = logger || new Loggers_1.NullLogger();
        ...
        pollXhr.onload = () => {
            if (pollXhr.status == 200) {
                if (this.onDataReceived) {
                    try {
                        if (pollXhr.response) {
                            this.logger.log(ILogger_1.LogLevel.Information, `(LongPolling transport) data received: ${pollXhr.response}`);
